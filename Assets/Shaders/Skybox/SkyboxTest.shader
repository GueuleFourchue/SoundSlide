	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float2 uv1 = sin(length(i.uv)*_Scale);
				float2 uv2 = sin((dot(i.view, i.normal)*_Scale));
				float2 uv = lerp(uv2, uv1, abs(dot(i.view, float3(0,0,1))));

				uv.x = atan2(i.vertexWorld.y, i.vertexWorld.x);
				uv.y += abs(sin(uv.x+ _Time.y*_Speed));
				if(length(uv1)>1){ uv.x = atan2(i.vertexWorld.y, i.vertexWorld.x); };
