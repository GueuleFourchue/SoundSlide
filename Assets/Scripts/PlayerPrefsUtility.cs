...
namespace Paladin.Framework.Utilities
{
    public static class PlayerPrefsUtility
    {
        ...
        public static bool[] GetBoolArray(string key, bool defaultValue, int defaultSize)
        {
            if (PlayerPrefs.HasKey(key))
            {
                bool[] temp = GetBoolArray(key);

                if (temp.Length < defaultSize)
                {
                    bool[] bArray = new bool[defaultSize];
                    for (int i = 0; i < defaultSize; i++)
                        bArray[i] = defaultValue;

                    for (int i = 0; i < Math.Min(temp.Length, defaultSize); i++)
                    {
                        bArray[i] = temp[i];
                    }
                    return bArray;
        ...
        public static int[] GetIntArray(string key, int defaultValue, int defaultSize)
        {
            if (PlayerPrefs.HasKey(key))
            {
                int[] temp = GetIntArray(key);

                if (temp.Length < defaultSize)
                {
                    int[] bArray = new int[defaultSize];
                    for (int i = 0; i < defaultSize; i++)
                        bArray[i] = defaultValue;
        ...
        public static float[] GetFloatArray(string key, float defaultValue, int defaultSize)
        {
            if (PlayerPrefs.HasKey(key))
            {
                float[] temp = GetFloatArray(key);

                if (temp.Length < defaultSize)
                {
                    float[] fArray = new float[defaultSize];
                    for (int i = 0; i < defaultSize; i++)
                        fArray[i] = defaultValue;
        ...
        public static string[] GetStringArray(string key, char separator, string defaultValue, int defaultSize)
        {
            if (PlayerPrefs.HasKey(key))
            {
                string[] temp = PlayerPrefs.GetString(key).Split(separator);

                if (temp.Length < defaultSize)
                {
                    string[] bArray = new string[defaultSize];
                    for (int i = 0; i < defaultSize; i++)
                        bArray[i] = defaultValue;
